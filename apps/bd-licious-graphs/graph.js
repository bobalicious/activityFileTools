...
(function (root) {
  ...
  // Width a rest lap takes when rests are compressed, however long it lasted.
  var REST_GAP_PX = 6;
  ...
  function renderChart(o) {
    ...
    var compressRests = !!o.compressRests;
    ...
    var xOf = timeToX(compressRests ? laps : [], xMin, xMax, left, plotW);
  ...
  // Maps activity time to x. Each rest lap given gets a fixed REST_GAP_PX slot,
  // time within it squashed linearly; everything else shares the remaining width
  // in proportion to its duration. With no rest laps it is a plain linear scale.
  function timeToX(laps, xMin, xMax, left, plotW) {
    var rests = laps.filter(function (l) { return l.isRest; }).map(function (l) {
      return [Math.max(xMin, l.startTime), Math.min(xMax, l.startTime + l.elapsedTime)];
    }).filter(function (r) { return r[1] > r[0]; }).sort(function (a, b) { return a[0] - b[0]; });
    var restTotal = rests.reduce(function (a, r) { return a + r[1] - r[0]; }, 0);
    // Never let the gaps take more than a third of the plot.
    var gapPx = rests.length ? Math.min(REST_GAP_PX, plotW / 3 / rests.length) : 0;
    var secPerPx = Math.max(1, xMax - xMin - restTotal) / (plotW - gapPx * rests.length);
    return function (t) {
      var px = 0, from = xMin;
      for (var i = 0; i < rests.length && t > rests[i][0]; i++) {
        var r = rests[i];
        px += (r[0] - from) / secPerPx;
        if (t < r[1]) return left + px + gapPx * (t - r[0]) / (r[1] - r[0]);
        px += gapPx; from = r[1];
      }
      return left + px + (t - from) / secPerPx;
    };
  }

  function mark(type, x, w, baseY, color, op, yStat, yAvg, yMin, yMax) {
    var s;
    if (type === 'bar') s = '<rect x="' + x.toFixed(1) + '" y="' + yStat.toFixed(1) + '" width="' + w.toFixed(1) + '" height="' + (baseY - yStat).toFixed(1) + '" fill="' + color + '" rx="3"/>';
    ...
    else if (type === 'range') s = '<rect x="' + x.toFixed(1) + '" y="' + yMax.toFixed(1) + '" width="' + w.toFixed(1) + '" height="' + Math.max(1, yMin - yMax).toFixed(1) + '" fill="' + color + '" fill-opacity="0.45" rx="3"/><line x1="' + x.toFixed(1) + '" y1="' + yAvg.toFixed(1) + '" x2="' + (x + w).toFixed(1) + '" y2="' + yAvg.toFixed(1) + '" stroke="' + color + '" stroke-width="2.5"/>';
    else s = '';
